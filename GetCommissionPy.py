...
# 0.1 баг - при любом символе в поле тип коммисии вылетает Валуе еррор
# 0.2 фича - вычисление дирекции сделать также как и имя из результирующей выборки
# 0.3 баг - если имя сборное из несклоьких частей ТДВ "СК "СПЕЙР" не верно переименовывает файл
# 0.4 баг - при выгрузке основной комиссии добавляется ДОСТРОКВА +
# 0.5 фича - спрашивать куда сохранить файл
# 0.6 баг - позволяет вводить пустое значение дирекции
# 0.7 фича - добавить выборку только по номреу акта
# Попробовать сделать программу выполняемой в цыкле
# чтобы каждый раз не вводить заново значения даты и периода
# Разница между файлами в передаче branchcode с двумя одинарными кавычками,
# надо переписать проверку даты(разрешить вводить сегодняшнее число)
# Сделать одну программу которая брала в зависимости от введеной даты выбирала
# какой скрипт использовать при запросе
...
if __name__ == "__main__":
    ...
    type_out = "_дострокова" if now_datetime.month == month_period else ""

    if chanel in [22, 18, 17, 15] and comm_type_gid == comm_type[1][0]:
        output_file = 'part_СПД_ІКП_{}дир{}.txt'.format('_'.join(["{:02}".format(code) for code
                                                                           in sorted(branch_code_lst)]), type_out)
    elif chanel in [22, 18]:
        output_file = 'part_СПД_{}дир{}.txt'.format('_'.join(["{:02}".format(code) for code
                                                                       in sorted(branch_code_lst)]), type_out)
    else:
        output_file = 'part_{}к_{}дир{}.txt'.format(chanel, '_'.join(["{:02}".format(code) for code
                                                                               in sorted(branch_code_lst)]), type_out)
